...
class Map:
    ...
    @classmethod
    def generate_random_obstacles(cls, num_static=2, num_dynamic=1):
        ...
        for _ in range(num_dynamic):
            while True:
                ...
                height = random.uniform(1, 3)
                new_obstacle = geo.Polygon([(x, y), (x + width, y), (x + width, y - height), (x, y - height)])
                # Kiểm tra xem chướng ngại vật mới có giao với các chướng ngại vật tĩnh hoặc động đã tồn tại hay không
    ...
    def update(cls, x):
        ...
        ] + cls.obstacles[2:]  # Giữ nguyên các chướng ngại vật tĩnh
